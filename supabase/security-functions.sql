...
-- La file d'attente est visible pour les activités approuvées.
-- L'ordre de la file est déterminé par followed_at.
drop policy if exists "Users can view their own follows"
on public.followers;

create policy "Anyone can view approved activity waiting queues"
on public.followers
for select
...
using (
    exists (
        select 1
        from public.activities
        where public.activities.id = public.followers.activity_id
          and public.activities.status = 'approved'
    )
);

-- Lorsqu'un utilisateur rejoint finalement une activité, il quitte
-- automatiquement la file d'attente de cette activité.
create or replace function public.join_activity(p_activity_id uuid)
returns void
language plpgsql
security definer
