...
function createUI(authMenu) {
    ...
    // Le réglage ne doit jamais être interprété comme un clic extérieur
    // au panneau, y compris sur mobile/tactile.
    for (const eventName of ["click", "pointerdown", "mousedown", "touchstart"]) {
        preferences.addEventListener(eventName, event => event.stopPropagation());
    }
    toggle.addEventListener("change", event => {
        event.stopPropagation();
        void setNotificationsPreference(toggle.checked);
    });
